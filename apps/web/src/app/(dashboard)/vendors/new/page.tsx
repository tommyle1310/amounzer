...
export default function NewVendorPage() {
  ...
  const createMutation = useMutation({
    mutationFn: (payload: Record<string, unknown>) => apiClient.post('/vendors', payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['vendors'] });
      router.push('/vendors');
    },
    onError: (err) => setError(err instanceof Error ? err.message : 'Lỗi tạo nhà cung cấp'),
  });

  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!code.trim()) {
      setError('Mã nhà cung cấp là bắt buộc');
      return;
    }
    if (!name.trim()) {
        ...
        <Card>
          ...
          <CardContent className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="code">Mã nhà cung cấp (tiểu khoản) *</Label>
              <Input
                id="code"
                value={code}
                onChange={(e) => setCode(e.target.value)}
                placeholder="VD: NCC001, VNPT"
                required
              />
            </div>
            <div className="space-y-2">
